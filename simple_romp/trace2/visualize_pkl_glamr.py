...
class TRACE(nn.Module):
    ...
    def save_results(self, outputs, tracking_results, kp3d_results, imgpaths):
        save_paths = preds_save_paths(self.save_path, prefix='trace_demo')
        visualize_predictions(outputs, imgpaths, self.FOV, save_paths.seq_save_dir, self.smpl_model_path)
        return
...
def main():
    ...
    if args.mode == 'video':
        ...
        # outputs = np.load("../results/trace_demo/trace_demo.npz", allow_pickle=True)['outputs'][()]
        # imgpaths = np.load("../results/trace_demo/trace_demo.npz", allow_pickle=True)['imgpaths'][()]
        imgpaths = np.asarray(os.listdir("/data/amogh/workspace/projects/ROMP/data/demo_seqs/Dynacam_office_seed1/frames"), dtype='U71')
        glamr_output = pkl.load(open("/data/amogh/workspace/projects/ROMP/data/demo_seqs/Dynacam_office_seed1/Dynacam_office_seed1.pkl", "rb"))

        outputs = {}
        seq_len = len(imgpaths)
        outputs['seq_len'] = seq_len
        outputs['reorganize_idx'] = [i for i in range(seq_len)]
        outputs['track_ids'] = [1 for i in range(seq_len)]
        outputs['cam_trans'] = glamr_output['']
        outputs['verts'] = 
        outputs['verts_camed_org'] = 
        # tracking_results = np.load("../results/trace_demo/trace_demo_tracking.npz", allow_pickle=True)['tracking'][()]
        tracking_results = None
        # kp3d_results = np.load("../results/trace_demo/trace_demo_tracking.npz", allow_pickle=True)['kp3ds'][()]
        kp3d_results = None
        # np.load("../results/trace_demo/trace_demo_tracking.npz", allow_pickle=True)['kp3ds'][()]
        breakpoint()
